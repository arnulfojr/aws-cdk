...
export = testCase({
  ...
  "backup vault with encryption key"(test: Test) {
    ...
    // THEN
    expect(stack).to(
      haveResource("AWS::Backup::BackupVault", {
        BackupVaultName: "MyVault",
        EncryptionKeyArn: {
  ...
  "backup vault with SNS Topic notification"(test: Test) {
    ...
    new BackupVault(stack, "MyVault", {
      backupVaultName: "MyVault",
      notificationTopic: new sns.Topic(stack, "MyTopic"),
      notificationEvents: [EventType.BACKUP_JOB_STARTED],
    });
    ...
    expect(stack).to(
      haveResource("AWS::Backup::BackupVault", {
        ...
        Notifications: {
          ...
          BackupVaultEvents: [EventType.BACKUP_JOB_STARTED],
        },
      })
    );
  ...
  "simple backup plan"(test: Test) {
    // GIVEN
    const stack = new cdk.Stack();
    const backupVault = new BackupVault(stack, "MyVault", {
      backupVaultName: "MyVault",
    });
    const schedule = Schedule.cron({ hour: "2" });
    const planRule = new PlanRule({ schedule, backupVault });
    ...
    expect(stack).to(
      haveResource("AWS::Backup::BackupPlan", {
        BackupPlan: {
          BackupPlanName: "ThePlanArises",
          BackupPlanRule: [
            {
  ...
  "add backup rules into backup plan"(test: Test) {
    ...
    // WHEN
    const myFirstRule = new PlanRule({
      ruleName: "MyFirstRule",
      schedule,
      backupVault,
    });
    ...
    backupPlan.addPlanRule(PlanRule.weekly(backupVault));
  ...
  "backup plan with rules and lifecycles"(test: Test) {
    // GIVEN
    const stack = new cdk.Stack();
    const backupVault = new BackupVault(stack, "BearVault", {
    ...
    backupPlan.addPlanRule(planRule);
  ...
  "backup plan with rule and window support"(test: Test) {
    // GIVE
    const stack = new cdk.Stack();
    const backupVault = new BackupVault(stack, "MyBackupVault", {
      backupVaultName: "MyVault",
    });
    const schedule = Schedule.cron({ hour: "4" });
    ...
    // WHEN
    const planZv1Rule = new PlanRule({
      ruleName: "Plan Z v1",
      schedule,
      completionWindow,
      startWindow,
  ...
  "backup plan with selection without given IAM Role"(test: Test) {
    ...
    expect(stack).to(
      haveResource("AWS::Backup::BackupSelection", {
        BackupPlanId: "Plan X",
        BackupSelection: {
          IamRoleArn: {
            "Fn::GetAtt": ["BackupSelectionRoleD4B27382", "Arn"],
          },
